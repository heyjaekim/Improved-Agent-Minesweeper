...
from minimizing_cost_agent import *
from minimizing_risk_agent import *
...
class Visual(tk.Frame):
    ...
    def click_next(self):
        ...
        if result != -1:
            ...
            self.filloutRemainSquares()
            print("WINNER WINNER CHICKEN DINNER")

    def filloutRemainSquares(self):
        for i in range(self.env.dim):
            for j in range(self.env.dim):
                if self.env.hidden_grid[i][j] == 0:
                    self.env.hidden_grid[i][j] = -1
                    self.label_matrix[i][j].configure(text="M",bg="red")
...
=======
    ...
    imp_agent = minimizingRiskAgent(rendered_grid, 2)
    imp_agent = improvedAgent(rendered_grid, 2)
>>>>>>> 6ae829a54568bdb3d70ce35830701669fe86a000
