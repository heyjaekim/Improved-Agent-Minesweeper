...
class minimizingRiskAgent(object):
...
def bonusQuestion(num_games, num_mines, dim):
    ...
    first_plot = ax.bar(x, avg_risk, width, color = 'limegreen')
    second_plot = ax.bar(x + width, avg_risk2, width, color = 'b')

    ax.set_xlabel("# OF THE MINE (MINE DENSITY)")
    ax.set_ylabel("Average Risk")
    plt.title("BQ: Comparison btw Slightly Improved Agents When It Comes To Minimize Risk")
    plt.xticks(x)
